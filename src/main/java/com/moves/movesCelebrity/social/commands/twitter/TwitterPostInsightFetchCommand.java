...
import com.moves.movesCelebrity.social.models.TwitterInsights;
...
public class TwitterPostInsightFetchCommand implements Command<ArrayList<Document>,String> {

    private Twitter twitter = TwitterFactory.getSingleton();
    private Logger logger = LoggerFactory.getLogger(TwitterPostInsightFetchCommand.class);
    ...
    public TwitterPostInsightFetchCommand() {
    }
    ...
    public ArrayList<Document> fetch(String screenName,Integer page , Integer count) throws TwitterException, IOException {
        logger.info("Twitter fetch function to fetch Post Insights called");
    ...
    public ArrayList<TwitterInsights> fetch() throws TwitterException, IOException {
        Integer page = 1;
        Integer count = 2000;
        Paging paging = new Paging();
        List<Status> statuses = null;
        ArrayList<TwitterInsights> posts;
        ...
        while(!statusEnd){
            paging.setPage(page++);
            logger.info("Paging " + paging.getPage());
            if(statuses == null){
                statuses = twitter.getHomeTimeline(paging);
            }else{
            }
            logger.info("Status count " + twitter.getHomeTimeline(paging).size());
            statusEnd = twitter.getHomeTimeline(paging).size() == 0 ? true : false;
        }
        posts = mapper.readValue(gson.toJson(statuses) , new TypeReference<List<TwitterInsights>>(){
        });
        return posts;
    }
